async function loadPost() {
  ...
  badges.className = "seller-badges";
  ...
  const viewListingsBtn = document.createElement("button");
  viewListingsBtn.className = "view-listings-btn";
  viewListingsBtn.textContent = `View all listings by ${seller?.name || "seller"}`;
  viewListingsBtn.onclick = () => window.openSellerProfile(post.userId);

  info.append(postedBy, postedOn, badges, viewListingsBtn);
  header.append(avatar, info);
  right.appendChild(header);

  // Post title / price / description
  ...
  if (post.price !== undefined) {
    ...
    h2.textContent = post.price === 0 ? "FREE" : `£${post.price}`;
  ...
  ["category", "subcategory", "area"].forEach(field => {
    if (post[field]) {
      const p = document.createElement("p");
      p.innerHTML = `<strong>${field.charAt(0).toUpperCase() + field.slice(1)}:</strong> ${post[field]}`;
      meta.appendChild(p);
    }
  });

  const pPosted = document.createElement("p");
  pPosted.innerHTML = `<strong>Posted:</strong> ${post.createdAt?.toDate().toLocaleDateString() || "Unknown"}`;
  const pViews = document.createElement("p");
  pViews.innerHTML = `<strong>Views:</strong> ${post.views ? post.views + 1 : 1}`;

  meta.append(pPosted, pViews);
  ...
  engageBtn.onclick = () => window.engagePost(window.selectedPostId);
  ...
  if (isOwner) {
    ...
    manageBtn.textContent = "Manage this ad in your dashboard";
    manageBtn.onclick = navigateToDashboard;
